...
router.get("/index", (req, res) => {
  ...
  db.query(userQuery, [username], (userErr, userResult) => {
    ...
    const balanceQuery =
      "SELECT balance_ron, balance_usd, balance_eur FROM accounts WHERE user_id = ?";
    db.query(balanceQuery, [userId], (balanceErr, balanceResult) => {
      if (balanceErr) {
        console.error("Error retrieving balances:", balanceErr);
        return res
          .status(500)
          .json({ message: "An error occurred while retrieving balances" });
      }
      ...
      const transactionQuery =
        "SELECT transaction_type, amount, recipient, date FROM transactions WHERE user_id = ? ORDER BY date DESC LIMIT 10";
      db.query(
        transactionQuery,
        [userId],
